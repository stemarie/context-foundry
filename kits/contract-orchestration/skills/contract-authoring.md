...
5. State delivery only when explicitly authorized. Select `candidate_only`, `merge_required`, or `human_approval_required` and bind base SHA, candidate branch/SHA, owner, and next decision. A local commit is not delivery. For `merge_required`, require a separately audited PR head, non-force merge, authenticated default-branch read-back, and post-merge checks; Candidate Auditor PASS alone never certifies integration, deployment, or milestone closure.
