#!/usr/bin/env python3
"""Task-bound, read-only V2 verification for a Foundry Delivery contract."""
...
def live_card(task_id: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or value.get("id") != task_id:
        raise ContractReadError("Delivery card response was malformed")
    if value.get("assignee") != PROFILE or not str(value.get("title", "")).startswith(TITLE_PREFIX):
        raise ContractReadError("assigned card is not a Foundry Delivery card")
    return value
...
def identity(card: dict[str, Any]) -> tuple[str, int]:
    ...
    if len(lines) != 4:
        raise ContractReadError("Delivery card must contain exactly four V2 routing lines")
    match = IDENTITY_RE.fullmatch(lines[0])
    if not match or lines[1] != "Role: Delivery" or not lines[2].startswith("Dependency: ") or not lines[2][12:] or not lines[3].startswith("Receipt pointer: ") or not lines[3][17:]:
        raise ContractReadError("Delivery V2 routing envelope is malformed")
    return match.group(1), int(match.group(2))


def execute(card: dict[str, Any]) -> dict[str, Any]:
    contract_id, revision = identity(card)
    contract = _run([CLIENT, "writer", "GET", f"/api/v1/contracts/{contract_id}"]).get("contract")
    frozen = _run([CLIENT, "writer", "GET", f"/api/v1/chain-contracts/{contract_id}/frozen"]).get("frozen_revision")
    ...
    return {"operation": "delivery-task-bound-v2-contract-read", "contract_id": contract_id, "revision": revision, "digest": frozen["digest"], "status": contract["status"]}
