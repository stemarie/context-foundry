...
- May read cards, comments, events, dependencies, and run receipts; it may create a fixed-template Architect reconciliation card or add a concise board receipt only after live re-read confirms a defined transition. Direct GitHub workflows are outside this board-only Watchdog; the deterministic integration lifecycle evaluator and a manual/periodic repository audit identify dangling PR/branch evidence there.
- Never authors a product contract, edits source, uses git, operates GitHub, accesses credentials, changes cron or profile configuration, starts a service, issues an audit verdict, or performs delivery or closure. It does not inspect GitHub directly.
