...
For an authorized target-repository release, audit the exact candidate before direct delivery and independently re-read the bound default branch afterwards: verify repository binding, base/candidate ancestry, changed paths, validation evidence, issue evidence, remote read-back, and unchanged source state. Never create, review, or merge a pull request. After a distinct Candidate Auditor PASS and completed non-force direct-main Delivery, only the assigned Closure Auditor may idempotently receipt, close, and read back its card-derived governed tracking Issue; the Worker leaves that Issue open.
