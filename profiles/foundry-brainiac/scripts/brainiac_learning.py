...
# In the canonical source checkout, source-local runtime state is forbidden. The
# same evaluator is copied into the isolated Brainiac profile at installation;
# that profile's state directory is explicitly runtime-only and is permitted.
ROOT = SCRIPT_PATH.parents[1] if (SCRIPT_PATH.parents[1] / "profiles").is_dir() else None
WINDOW_DAYS = 30
HIGH_SEVERITY = "high"
...
BRAINIAC_COMMAND = (
    ...
    "--provider", "openai-codex", "--model", "gpt-5.6-terra", "--toolsets", "bot_room", "--quiet",
)
...
def build_prompt(kind: str, invocation_key_value: str, evidence: dict[str, Any]) -> str:
    ...
    return json.dumps({
        ...
        "evidence": evidence,
        "evidence_origin": "Deterministic bridge: this is an allowlisted read-only row from MariaDB foundry_incident_log, not chat recollection or user-supplied evidence.",
...
def execute_qualified(
    ...
    *,
    ...
    except (BrainiacInputError, subprocess.SubprocessError, TimeoutError, OSError) as exc:
        # Keep a bounded diagnostic class only; never persist command output or
        # model text at this boundary.
        receipt = {"invocation_key": invocation_key_value, "kind": kind, "outcome": "failed", "reason": type(exc).__name__}
        with connect(state_path) as db:
